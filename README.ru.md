...
Проект настроен с использованием Prometheus для сбора метрик, Grafana для их визуализации и Node-exporter для метрик на уровне хоста, а также структурированного логирования с помощью Zap.

### Сбор метрик

Метрики собираются из различных компонентов приложения и хост-системы:

*   **Метрики системы-хоста:** Собираются Node-exporter'ом, включают нагрузку на CPU, использование RAM, Load Average (1м, 5м, 15м) и Network I/O.
*   **Метрики приложения:** Предоставляются приложением через эндпоинт `/metrics` (собираются Prometheus) и определены в `internal/pkg/metrics/metrics.go`. К ним относятся:
    ...
    *   **Метрики потребителя:**
        ...
        *   `consumer_processing_latency`: гистограмма длительности обработки сообщений.
        *   `consumer_lag`: показатель текущего отставания потребителя.
        *   `consumer_dlq_sent_total`: общее количество сообщений, отправленных в Dead-Letter Queue.
    *   **Метрики кэша:**
    ...
    *   **Метрики базы данных:**
        *   `db_response_time`: гистограмма времени ответа базы данных с метками по операции.
...
*   **Prometheus:**
    *   Настроен на сбор метрик с сервиса `app` (основной API потребителя) по адресу `http://app:8080/metrics` и Node-exporter по адресу `http://node-exporter:9100/metrics`, как определено в `configs/prometheus/prometheus.yml`.
    *   Доступен на хосте через порт `9090` (настраивается через `PROMETHEUS_PORT` в `.env`).

*   **Grafana:**
    *   Предварительно настроен с дашбордом (`configs/grafana/provisioning/dashboards/dashboard.json`), который визуализирует ключевые метрики приложения и метрики хост-системы.
